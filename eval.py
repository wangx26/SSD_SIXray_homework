...
parser.add_argument('--imagesetfile',
                    default=osp.join(SIXRAY_ROOT, 'test.txt'), type=str,
...
annopath = os.path.join(args.SIXray_root, 'sixray_eval/Annotation')
imgpath = os.path.join(args.SIXray_root, 'sixray_eval/Image')
...
def parse_rec(filename, imgname, imgpath):
    # filename = filename.replace('.xml', '.txt')
    filelist = os.listdir(filename)
    if 'core_battery' + imgname + '.txt' in filelist:
        filename_ = filename + '/core_battery' + imgname + '.txt'
    else:
        filename_ = filename + '/coreless_battery' + imgname + '.txt'
    imagename1 = imgpath + '/core_battery' + imgname + '.jpg'
    imagename2 = imgpath + '/coreless_battery' + imgname + '.jpg'
    objects = []
    # 还需要同时打开图像，读入图像大小
    ...
    with open(filename_, "r", encoding='utf-8') as f1:
